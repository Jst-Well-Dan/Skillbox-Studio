...
export const MCPManager: React.FC<MCPManagerProps> = ({
  ...
  const [originalServers, setOriginalServers] = useState<MCPServer[]>([]);
  ...
  useEffect(() => {
    if (originalServers.length > 0) {
      const applyTranslation = async () => {
        if (language === 'zh') {
          const translatedServers = await translatePlugins(originalServers);
          setServers(translatedServers);
        } else {
  ...
  const loadServers = async (forceRefresh = false) => {
    ...
    try {
      ...
      setOriginalServers(serverList);

      // Apply translation if in Chinese mode
      if (language === 'zh') {
        const translatedServers = await translatePlugins(serverList);
        setServers(translatedServers);
      } else {
