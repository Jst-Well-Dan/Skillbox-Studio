...
export function usePromptExecution(config: UsePromptExecutionConfig): UsePromptExecutionReturn {
  ...
  const handleSendPrompt = useCallback(async (
    ...
    console.log('[usePromptExecution] ========================================');
    console.log('[usePromptExecution] handleSendPrompt called with:');
    console.log('[usePromptExecution] prompt:', prompt);
    console.log('[usePromptExecution] model:', model);
    console.log('[usePromptExecution] projectPath:', projectPath);
    console.log('[usePromptExecution] projectPath type:', typeof projectPath);
    console.log('[usePromptExecution] projectPath length:', projectPath?.length);
    console.log('[usePromptExecution] isLoading:', isLoading);
    console.log('[usePromptExecution] ========================================');

    if (!projectPath) {
      console.error('[usePromptExecution] ❌ Validation failed: projectPath is empty!');
      setError("请先选择项目目录");
      return;
    }
      ...
      if (!isListeningRef.current && isActive) {
        ...
        async function handleStreamMessage(payload: string, currentTranslationResult?: TranslationResult) {
          try {
            ...
            // ⭐ Detect AskUserQuestion tool use for interactive prompts
            if (message.type === 'assistant' && message.message?.content) {
              const content = message.message.content;
              if (Array.isArray(content)) {
                const askQuestionTool = content.find((item: any) =>
                  item.type === 'tool_use' && item.name === 'AskUserQuestion'
                );

                if (askQuestionTool) {
                  console.log('[AskUserQuestion] Detected interactive question:', askQuestionTool);
                  // Attach question data to message for rendering
                  message.pendingQuestion = {
                    toolUseId: askQuestionTool.id,
                    questions: askQuestionTool.input?.questions || [],
                  };
                }
              }
            }
