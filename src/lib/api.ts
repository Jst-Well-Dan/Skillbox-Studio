 ...
 */
export type PermissionMode = 'Interactive' | 'AcceptEdits' | 'ReadOnly' | 'Plan';
...
export interface ClaudePermissionConfig {
  ...
  /** Whether to skip all permission checks (dangerous!) */
...
/**
 * Plugin definition in marketplace
 */
...
export const api = {
  ...
  async executeClaudeCode(projectPath: string, prompt: string, model: string, planMode?: boolean, maxThinkingTokens?: number): Promise<void> {
    // 🔍 DEBUG: Log the invoke call
    console.log('[API] 🚀 Calling invoke("execute_claude_code") with:', {
      projectPath,
      prompt,
      model,
      planMode,
  ...
  async continueClaudeCode(projectPath: string, prompt: string, model: string, planMode?: boolean, maxThinkingTokens?: number): Promise<void> {
    console.log('[API] 🚀 Calling invoke("continue_claude_code") with:', {
      projectPath,
      prompt,
      model,
      planMode,
  ...
  async resumeClaudeCode(projectPath: string, sessionId: string, prompt: string, model: string, planMode?: boolean, maxThinkingTokens?: number): Promise<void> {
    console.log('[API] 🚀 Calling invoke("resume_claude_code") with:', {
      projectPath,
      sessionId,
      prompt,
      model,
  ...
  /**
   * ⭐ Responds to an AskUserQuestion prompt from Claude CLI
   * Enables bidirectional communication for interactive questions during execution
   * @param sessionId - The Claude session ID
   * @param toolUseId - The tool_use_id from the AskUserQuestion tool call
   * @param answers - User's answers to the questions (key = question index, value = selected option(s))
   * @returns Promise that resolves when the response is sent to Claude CLI
   */
   ...
   * - 有新会话：<500ms（增量更新）
   *
   * @param days - Optional number of days to filter (null for all time)
   * @returns Promise resolving to usage statistics
   */
  async getUsageStatsCached(days?: number): Promise<UsageStats> {
    try {
      return await invoke<UsageStats>("get_usage_stats_cached", { days: days || null });
    } catch (error) {
  ...
  /**
   * List all available plugin marketplaces
   */
  async listPluginMarketplaces(): Promise<MarketplaceInfo[]> {
    try {
      return await invoke<MarketplaceInfo[]>("list_plugin_marketplaces");
    } catch (error) {
      console.error("Failed to list plugin marketplaces:", error);
      throw error;
    }
  },
   ...
   */
  async listMarketplacePlugins(marketplaceName: string): Promise<PluginMetadata[]> {
    try {
      return await invoke<PluginMetadata[]>("list_marketplace_plugins", { marketplaceName });
    } catch (error) {
      console.error("Failed to list marketplace plugins:", error);
      throw error;
    }
  },

  /**
   * Install a plugin to project-level directory
   */
  async installPluginToProject(
    ...
    try {
      return await invoke<string>("install_plugin_to_project", {
        projectPath,
        marketplaceName,
        pluginName,
      });
  ...
  /**
   * Uninstall a plugin from project-level directory
   */
  async uninstallPluginFromProject(
    projectPath: string,
    marketplaceName: string,
    pluginName: string
  ): Promise<string> {
    try {
      return await invoke<string>("uninstall_plugin_from_project", {
        projectPath,
        marketplaceName,
        pluginName,
      });
   ...
   */
  async listProjectInstalledPlugins(projectPath: string): Promise<PluginMetadata[]> {
    try {
      return await invoke<PluginMetadata[]>("list_project_installed_plugins", { projectPath });
    } catch (error) {
  ...
  /**
   * Check if a plugin is installed in the project
   */
  async isPluginInstalled(
    ...
    try {
      return await invoke<boolean>("is_plugin_installed", {
        projectPath,
        marketplaceName,
        pluginName,
      });
    } catch (error) {
      console.error("Failed to check plugin installation:", error);
      throw error;
    }
  },
  ...
  /**
   * Get all commands available in the project (project-level + system-level)
   */
  async getProjectCommands(projectPath: string): Promise<CommandInfo[]> {
    try {
      return await invoke<CommandInfo[]>("get_project_commands", {
        projectPath,
      });
    } catch (error) {
      console.error("Failed to get project commands:", error);
      throw error;
    }
  },

  /**
   * Get all skills available in the project (project-level + system-level)
   */
  async getProjectSkills(projectPath: string): Promise<SkillInfo[]> {
    try {
      return await invoke<SkillInfo[]>("get_project_skills", {
        projectPath,
      });
    } catch (error) {
  ...
  /**
   * Get all agents available in the project (project-level + system-level)
   */
  async getProjectAgents(projectPath: string): Promise<AgentInfo[]> {
    try {
      return await invoke<AgentInfo[]>("get_project_agents", {
        projectPath,
      });
    } catch (error) {
  ...
  async listKnownMarketplaces(): Promise<MarketplaceDetail[]> {
    try {
      return await invoke<MarketplaceDetail[]>("list_known_marketplaces");
    } catch (error) {
      console.error("Failed to list known marketplaces:", error);
      throw error;
    }
  },

  /**
   * Add a new marketplace (GitHub URL or local path)
   */
  async addMarketplace(source: string): Promise<string> {
    try {
      return await invoke<string>("add_marketplace", { source });
    } catch (error) {
  ...
  async removeMarketplace(name: string): Promise<string> {
    try {
      return await invoke<string>("remove_marketplace", { name });
    } catch (error) {
  ...
  async refreshMarketplace(name: string): Promise<string> {
    try {
      return await invoke<string>("refresh_marketplace", { name });
    } catch (error) {
  ...
  /**
   * Read file content (for Plugin system)
   */
  ...
  /**
   * Ensure Plugin workspace exists (create if doesn't exist)
  ...
  async ensurePluginWorkspace(pluginName: string): Promise<string> {
    ...
    const workspacePath = await join(home, 'Documents', 'Claude-Workspaces', pluginName);
  ...
  /**
   * Setup plugin in workspace (create .claude/plugins symlink)
  ...
  async setupPluginInWorkspace(pluginInstallPath: string, workspacePath: string): Promise<void> {
    ...
    const claudeDir = await join(workspacePath, '.claude');
    const pluginsDir = await join(claudeDir, 'plugins');

    // 创建.claude/plugins目录
    await this.createDirectory(pluginsDir);

    // 创建符号链接
    const pluginDirName = await basename(pluginInstallPath);
    const linkTarget = await join(pluginsDir, pluginDirName);

    // 检查符号链接是否已存在
    const linkExists = await this.pathExists(linkTarget);
    if (!linkExists) {
      await this.createSymlink(pluginInstallPath, linkTarget);
    }
  },

  /**
   * Get the current Claude permission configuration
   * @returns Promise resolving to the permission configuration
   */
  async getClaudePermissionConfig(): Promise<ClaudePermissionConfig> {
    try {
      return await invoke<ClaudePermissionConfig>("get_claude_permission_config");
    } catch (error) {
  ...
  /**
   * Update the Claude permission configuration
   * @param config - The new permission configuration
   * @returns Promise resolving when the configuration is updated
   */
  async updateClaudePermissionConfig(config: ClaudePermissionConfig): Promise<void> {
    try {
      await invoke("update_claude_permission_config", { permissionConfig: config });
    } catch (error) {
  ...
  /**
   * Run diagnostic test for Claude CLI
   * Returns a detailed report about Claude CLI availability and functionality
   */
  async diagnosticClaudeCli(): Promise<string> {
    try {
  ...
  /**
   * Test Tauri event system
   * Emits a test event to verify event handling
   */
  async diagnosticTestEvent(): Promise<void> {
    try {
