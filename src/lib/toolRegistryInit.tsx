...
export function initializeToolRegistry(): void {
  ...
  const buildPlanModeWidgetProps = (renderProps: ToolRenderProps, mode: 'enter' | 'exit') => {
    const reasonSources = [
      renderProps.input?.reason,
      renderProps.input?.message,
      renderProps.input?.prompt,
      renderProps.input?.context,
    ];

    const reason =
      reasonSources
        .map((value) => extractStringContent(value ?? ''))
        .find((text) => Boolean(text.trim())) || undefined;

    const resultMessage = extractStringContent(
      (renderProps.result?.content as any)?.message ??
        (renderProps.result?.content as any)?.detail ??
        renderProps.result?.content ??
        ''
    );
  ...
  const tools: ToolRenderer[] = [
    ...
    },
    {
      ...
      render: createToolAdapter(PlanModeWidget, (props) => buildPlanModeWidgetProps(props, 'enter')),
      description: '开启 Plan Mode',
    },
    {
      name: 'exitplanmode',
      pattern: /^exit[-_]?plan[-_]?mode$/i,
      render: createToolAdapter(PlanModeWidget, (props) => buildPlanModeWidgetProps(props, 'exit')),
