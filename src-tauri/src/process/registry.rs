...
pub struct ProcessHandle {
    ...
    /// ⭐ stdin handle for bidirectional communication with Claude CLI (AskUserQuestion support)
    pub stdin: Option<Arc<tokio::sync::Mutex<Option<tokio::process::ChildStdin>>>>,
    #[cfg(windows)]
...
impl ProcessRegistry {
    ...
    pub fn register_claude_session(
        ...
        let process_handle = ProcessHandle {
            ...
            stdin: None, // ⭐ Will be set later in spawn_claude_process
            #[cfg(windows)]
            job_object,
        };
    ...
    fn register_process_internal(
        ...
        let process_handle = ProcessHandle {
            ...
            live_output: Arc::new(Mutex::new(String::new())),
            stdin: None, // ⭐ Agents don't need stdin support
            #[cfg(windows)]
            job_object,
        };
    ...
    /// ⭐ Set stdin for a Claude session (for AskUserQuestion support)
    pub fn set_claude_session_stdin(
        &self,
        session_id: &str,
        ...
        for handle in processes.values_mut() {
            if let ProcessType::ClaudeSession { session_id: sid } = &handle.info.process_type {
                if sid == session_id {
                    ...
                    log::info!("Set stdin for Claude session: {}", session_id);
                    return Ok(());
                }
            }
        }

        Err(format!("Claude session not found: {}", session_id))
    }

    /// ⭐ Get stdin handle for a Claude session (for responding to AskUserQuestion)
    pub fn get_claude_session_stdin(
        &self,
        session_id: &str,
    ) -> Result<Option<Arc<tokio::sync::Mutex<Option<tokio::process::ChildStdin>>>>, String> {
        let processes = self.processes.lock().map_err(|e| e.to_string())?;

        for handle in processes.values() {
            if let ProcessType::ClaudeSession { session_id: sid } = &handle.info.process_type {
                if sid == session_id {
                    return Ok(handle.stdin.clone());
                }
            }
        }
