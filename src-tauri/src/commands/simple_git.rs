...
pub fn ensure_git_repo(project_path: &str) -> Result<(), String> {
    ...
    config_name.args(["config", "user.name", "Skillbox Studio"]);
    ...
    config_email.args(["config", "user.email", "ai@skillbox-studio"]);
    ...
    commit_cmd.args([
        ...
        "--allow-empty",
        "-m",
        "[Skillbox Studio] Initial commit - preserving existing code",
    ]);
